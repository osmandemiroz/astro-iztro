...
class BaZiEngine {
  ...
  static Map<String, dynamic> calculateBaZi({
    ...
    try {
      ...
      final yearPillar = _calculateYearPillar(birthDate.year);
      ...
      // Determine day master and analyze strength
      final dayMaster = dayPillar['stem'] as String;
      final dayMasterAnalysis = _analyzeDayMaster(dayMaster, elementCounts);

      // Calculate hidden stems
      final hiddenStems = _calculateHiddenStems([
        yearPillar,
        monthPillar,
        dayPillar,
        hourPillar,
      ...
      final analysis = _generateBaZiAnalysis(
        yearPillar,
        monthPillar,
        dayPillar,
        hourPillar,
  ...
  static Map<String, dynamic> _calculateDayPillar(DateTime birthDate) {
    ...
    final daysSinceEpoch = birthDate.difference(DateTime(1970)).inDays;
  ...
  static int _getMonthStemIndex(String yearStem, int month) {
    ...
    return (yearStemIndex + 2 + month - 1) % 10;
  }
  ...
  static int _getHourStemIndex(String dayStem, int timeIndex) {
    ...
    return (dayStemIndex + timeIndex) % 10;
  }
  ...
  static Map<String, int> _calculateElementCounts(
    ...
    for (final pillar in pillars) {
      // Count stem element
      final stemElement = pillar['stemElement'] as String;
      counts[stemElement] = (counts[stemElement] ?? 0) + 1;

      // Count branch element
      final branchElement = pillar['branchElement'] as String;
      counts[branchElement] = (counts[branchElement] ?? 0) + 1;

      // Count hidden stem elements
      final hiddenStems = pillar['hiddenStems'] as List<String>;
  ...
  static List<String> _getFavorableElements(String dayMasterElement) {
    ...
    const favorableMap = {
      ...
      '火': ['木', '土'], // Fire benefits from Wood (fuel) and Earth (foundation)
      '土': ['火', '金'], // Earth benefits from Fire (warmth) and Metal (minerals)
      '金': ['土', '水'], // Metal benefits from Earth (ore) and Water (cooling)
      '水': ['金', '木'], // Water benefits from Metal (containers) and Wood (flow)
    };
  ...
  static Map<String, List<String>> _calculateHiddenStems(
    ...
    for (var i = 0; i < pillars.length; i++) {
      ...
      hiddenStems[pillarNames[i]] = pillar['hiddenStems'] as List<String>;
    }
  ...
  static List<String> _generateRecommendations(
    ...
    final favorableElements =
        dayMasterAnalysis['favorableElements'] as List<String>;
  ...
  static Map<String, dynamic> _assessCompatibility(
    ...
    final yearElement = yearPillar['stemElement'] as String;
    final monthElement = monthPillar['stemElement'] as String;
    final dayElement = dayPillar['stemElement'] as String;
    final hourElement = hourPillar['stemElement'] as String;
    ...
    return {
      ...
      'analysis': 'Diverse elements suggest adaptability and growth potential',
    };
  }
